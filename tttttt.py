# -*- coding: utf-8 -*-
"""
Created on Thu Dec  9 12:03:45 2021
...
def COBE(x,a1):
    return (2*h*x**3)/(c**2 * ( np.exp(h*x/(k*a1)) - 1 ))


# Hacemos el ajuste de la función por el método de Levenberg-Marquardt
Pfit,Pcov=scipy.optimize.curve_fit(COBE,nu_SI,I_SI,p0=[1,1,1],sigma=SigmaI_SI,)

# Obtenemos los parámetros del arreglo
a1=Pfit
# Imprimimos nuestros parámetros físicos
print('T=',a1)
print(Pcov)

# Graficamos
plt.figure()
# Primero datos y barras de error
plt.errorbar(nu_SI,I_SI,SigmaI_SI,fmt='o',ecolor='r',lolims=False,uplims=False)
plt.title('COBE con barras de error')
# Ahora la gráfica de la función con los parámetros calculados
Energia=np.linspace(nu_SI[0],nu_SI[8],100)
evalBW=BW(Energia,a1)
plt.plot(Energia,evalBW,'-g')
plt.show()